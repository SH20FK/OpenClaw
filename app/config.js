...
export const config = {
  ...
  // ── File-flow (creative tasks) ─────────────────────────────────────────────
  fileFlowEnabled: (process.env.FILE_FLOW_ENABLED || "true").trim().toLowerCase() !== "false",
  // Raised default so larger single-file pages fit without getting truncated as often
  fileFlowMaxTokens: Number.parseInt(process.env.FILE_FLOW_MAX_TOKENS || "10000", 10),
...
function validateSharedConfig() {
  if (!config.companionServerUrl) throw new Error("Missing COMPANION_SERVER_URL in .env");
  if (!config.projectRoot) throw new Error("Missing PROJECT_ROOT in .env");
  if (!config.companionDeviceId) throw new Error("Missing COMPANION_DEVICE_ID in .env");
  if (!config.companionName) throw new Error("Missing COMPANION_NAME in .env");
  if (!config.publicBaseUrl) throw new Error("Missing PUBLIC_BASE_URL in .env");
  if (!config.apiHost) throw new Error("Missing API_HOST in .env");
  if (!Number.isFinite(config.apiPort)) throw new Error("API_PORT must be a valid number");
...
export function validateBotConfig() {
  ...
  if (!config.botToken) throw new Error("Missing TGBOT_API_KEY in .env");
